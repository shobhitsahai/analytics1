...
#Model
tree1 = rpart(Sales ~ . , data=data, method='anova' ) # for regression tree, method is ANOVA
tree1
rpart.plot(tree1, cex=.8)

#this is large tree, so prune it: check cp
printcp(tree1)
#cp value should be chosen such that xerror is least [CP = COMPLEXITY PARAMETER]
...
(predictedSales=predict(prunetree, newdata=testdata, type='vector')) #for regression tree use a vector
...
(predict(prunetree, newdata=testdata, type='class'))  # for classification tree use a CLASS
#ERROR - use vector
